...
name: audit
description: >
  Audit codebase against documentation, detect gaps, and reconcile to stay in sync. Not for fixing bugs or implementing new features.
version: "1.0.0"
...
* List tasks via `local-memory-mcp` MCP tools `task-active` (navigation), `task-list` (extended search), or `task-detail` (full content)
...
* `local-memory-mcp` MCP tools `task-detail`
